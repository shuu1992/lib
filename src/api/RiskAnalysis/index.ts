...
import { IList, IInfo, IAdd, IEdit, IDel, IAnalyze } from './req';
import { IResInfo, IResAnalyze } from './res';
/** List*/
...
/** Analyze*/
export async function apiAnalyze(postData: IAnalyze) {
  const config: AxiosRequestConfig = {
    url: '/analyze',
    method: 'GET',
    params: omitBy(postData, (v: any) => v === ''),
  };
  return await axios.request<any, PageRes<IResAnalyze>>(config);
}
