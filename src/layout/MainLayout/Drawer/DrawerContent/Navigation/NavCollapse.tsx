...
interface Props {
  ...
  setSelectedItems: React.Dispatch<React.SetStateAction<string | number | undefined>>;
...
const NavCollapse = ({
  ...
  const { drawerOpen, selectedItem, selectedID } = menuState;
  const { menuOrientation } = useConfig();
  const Navigation = useNavigate();
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<string | number | null | undefined>(null);
  ...
  function fcFindParentId(
    ...
    for (const node of tree) {
      if (node.id === targetId) {
        return parentId; // 找到目标ID时返回父级ID
      }
      if (node.children) {
        const result = fcFindParentId(node.children, targetId, node.id);
        if (result !== null) {
          return result; // 如果在子节点中找到目标ID，返回该结果
        }
      }
    }
    return null; // 未找到目标ID时返回null
  }
  ...
  }, [selectedItems, level, selected, miniMenuOpened, drawerOpen, selectedLevel, pathname]);

  useEffect(() => {
    if (selectedItem === null) return;
    setSelectedItems(selectedItem.pid);
  }, [selectedItem]);

  useEffect(() => {
    if (pathname === menu.url) {
      setSelected(selectedItem.pid);
    }
  }, [pathname]);

  const checkOpenForParent = (child: NavItemType[], id: string | number) => {
  ...
  useEffect(() => {
    if (menu.url === pathname) {
      dispatch(actionActiveItem({ openItem: [menu.id] }));
      setSelected(menu.id);
      setAnchorEl(null);
      setOpen(true);
    }
  ...
  const menuIcon = menu.icon ? (
    <AntIcon iconName={menu.icon} style={{ fontSize: drawerOpen ? '1rem' : '1.25rem' }} />
  ) : (
    borderIcon
  );
              ...
              <ListItemText
                primary={
                  <Typography variant="h6" color={selected === menu.id ? 'primary' : textColor}>
                    {menu.name === '' ? t(`${menu.title}`) : menu.name}
                  </Typography>
                }
                secondary={
                  menu.caption && (
