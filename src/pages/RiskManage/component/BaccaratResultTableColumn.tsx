...
export default function BaccaratResultTableColumn({ row }: { row: any }) {
  ...
  const numbersAry =
    row.getValue('numbers') === ''
      ? [0, 0, 0, 0, 0, 0]
      : row.getValue('numbers').split(',').map(Number);
  ...
  return (
    <Grid container spacing={0.5} alignItems="center" sx={{ minWidth: 250 }}>
      <Grid xs={6}>{t('sys.player')}</Grid>
      <Grid xs={6}>{t('sys.banker')}</Grid>
