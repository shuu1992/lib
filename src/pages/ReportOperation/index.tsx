...
import TreeSearch from './component/TreeSearch';
...
const ReportOperation = ({ pageHocProps = defaultPageHocProps }: WithPageHocProps) => {
  ...
  const [parentSearch, setParentSearch] = useState<ParentSearchType>({
    ...
    username: '',
    agent_paths: '',
    report_date1: getDateRange(sysTime, 'thisWeek')[0].format('YYYY-MM-DD'),
    report_date2: getDateRange(sysTime, 'thisWeek')[1].format('YYYY-MM-DD'),
  }); // 父層搜尋條件
  ...
  return (
    <Grid sx={{ mt: 2 }}>
      ...
      <Grid sx={{ mt: 2 }}>
        <TreeSearch treeAgent={parentSearch} setTreeAgent={setParentSearch} />
      </Grid>
      <Grid sx={{ mt: 2 }}>
        {step === 'operation' && (
          <Operation
