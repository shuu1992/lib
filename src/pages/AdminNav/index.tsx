...
import { apiList, apiInfo } from '@api/AdminNav';
import { IList, IInfo, IAdd, IEdit, IDel } from '@api/AdminNav/req';
...
import PageTable from '@src/components/muiTable/Tree';
...
const AdminNav = ({ pageHocProps = defaultPageHocProps }: WithPageHocProps) => {
  const {
    theme,
    menuState,
    globalState,
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: t('adminNav.route'),
        accessorKey: 'route',
        enableClickToCopy: true,
        muiFilterTextFieldProps: { placeholder: t('adminNav.route') },
        Cell: ({ row }: { row: any }) => {
          const route = row.getValue('route');
          const routeList = route ? route.split(',') : [];
          const isMultiline = routeList.length > 1;
          return (
            <div style={{ display: 'flex', flexDirection: isMultiline ? 'column' : 'row' }}>
      ...
      },
      {
        ...
        enableColumnFilter: false,
        Cell: ({ row }: { row: any }) => {
          return <AntIcon iconName={row.getValue('icon')} style={{ fontSize: '1.25rem' }} />;
        },
      },
      {
        header: t('adminNav.url'),
        accessorKey: 'url',
        enableColumnFilter: false,
      },
      {
        ...
        Cell: ({ row }: { row: any }) => {
          return (
            <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
              {allPermission.includes('admin_nav.store') && (
              ...
              {allPermission.includes('admin_nav.update') && (
              ...
              {allPermission.includes('admin_nav.destroy') && (
  ...
  return (
    <Grid>
      <PageTable
        ...
        addCfg={{
          flag: allPermission.includes('admin_nav.store'),
          anim: Boolean(editLoading) || loadingFlag,
          setFlag: () => {
            Modal.open(Add, {
              defaultData: { pid: null, id: 0 },
              fetchData: async () => {
                fcGetList({ ...pgCfg, searchCfg: { ...searchCfg, ...tSearchCfg } });
              },
            });
...
export default withPageHoc(AdminNav);
