...
const RiskAnalysis = ({ pageHocProps = defaultPageHocProps }: WithPageHocProps) => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: t('riskAnalysis.count'),
        accessorKey: 'count',
        enableColumnFilter: false,
        Cell: ({ row }: { row: any }) => {
          return <div> {fcMoneyFormat(row.getValue('count'))}</div>;
        },
      },
      {
        header: t('riskAnalysis.bet_total'),
      ...
      {
        header: t('riskAnalysis.same_count'),
        accessorKey: 'same_count',
        enableColumnFilter: false,
        Cell: ({ row }: { row: any }) => {
          return <div> {fcMoneyFormat(row.getValue('same_count'))}</div>;
        },
      },
      {
        header: t('riskAnalysis.same_bet_total'),
        accessorKey: 'same_bet_total',
        enableColumnFilter: false,
        Cell: ({ row }: { row: any }) => {
          return <div> {fcMoneyFormat(row.getValue('same_bet_total'))}</div>;
        },
      },
      {
        header: t('riskAnalysis.game_rate'),
        accessorKey: 'game_rate',
        enableColumnFilter: false,
        Cell: ({ row }: { row: any }) => {
          return <div> {fcMoneyDecimalFormat(row.getValue('game_rate'))}%</div>;
        },
      },
      {
        header: t('riskAnalysis.total_rate'),
        accessorKey: 'total_rate',
        enableColumnFilter: false,
        Cell: ({ row }: { row: any }) => {
          return <div> {fcMoneyDecimalFormat(row.getValue('total_rate'))}%</div>;
        },
      },
    ],
  ...
  return (
    <div>
      {loadingFlag && <Loader />}
      <Grid container xs={12} spacing={2.5} mt={2} mb={2}>
        <Grid xs={12} md={2}>
          <SearchOutLine
        ...
        <Grid xs={12} md={1}>
          ...
          >
            {collapse ? t('sys.collapse') : t('sys.expand')}
          </Button>
        </Grid>
      </Grid>
      {/* Chart */}
      {collapse && (
        <Grid container xs={12} spacing={0} mt={2} mb={4}>
          ...
          <Grid xs={12} md={4} p={2}>
            ...
            <Grid xs={12} sx={{ height: '50px' }} mt={2}>
              <BarChart win={analyzeObj.win_avg} lose={analyzeObj.lose_avg} disableTotal={true} />
            </Grid>
          </Grid>
          <Grid xs={12} md={4} p={2}>
            <Typography variant="h5" textAlign="center">
