...
import { dayType, getDateRange } from '@utils/date';
// material-ue
...
function SearchDateReportPicker({
  ...
  const shortcutsItems = useMemo(() => {
    return shortcutsItemList.map((type: string) => ({
      label: t(`cp.${type}`),
      getValue: () => {
        const date = getDateRange(sysTime, type as dayType);
        setendValue(date[1]);
        setstartValue(date[0]);
      },
    }));
  }, [shortcutsItemList, t, sysTime]);
...
export default SearchDateReportPicker;
